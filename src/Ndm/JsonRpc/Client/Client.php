...
class Client
{
    ...
    /**
     * @var \Ndm\JsonRpc\Client\Transport\TransportInterface
     */
    private $transport;

    /**
     * @var \Ndm\JsonRpc\Core\ResponseParser
     */
    private $parser;
     ...
     *
     *
     * @return null|\Ndm\JsonRpc\Core\Response|\Ndm\JsonRpc\Core\ResponseError
     */
    ...
    public function call($method, $arguments = null)
    {
        ...
        $response = $this->send(new Core\Request($method, $arguments, $id));
        ...
        if ($response instanceof Core\ResponseError) {
            ...
            $responseException = new Exception\ClientResponseException(var_export($response->error,true));
            if ($response->id !== $id) {
        ...
        assert('$response instanceof \\Ndm\\JsonRpc\\Core\\Response');
    ...
    public function notify($method, $arguments = null)
    {
        ...
        if ($response !== null) {
            ...
            if ($response instanceof Core\ResponseError) {
                throw new Exception\ClientResponseException(var_export($response->error,true));
            }
